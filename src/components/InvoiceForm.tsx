...
export const InvoiceForm = forwardRef<InvoiceFormHandle, InvoiceFormProps>(
  function InvoiceForm({ invoice, isPreview }, ref) {
    const { invoices, settings: appSettings } = useApp();
    const [formData, setFormData] = useState<Invoice>(() => {
      const s = appSettings;
      return {
        id: invoice?.id || Date.now().toString(),
        invoiceNo:
          invoice?.invoiceNo || getNextInvoiceNumber(invoices, appSettings),
        ...
        lineItems: invoice?.lineItems || [
          { id: "1", description: "", price: 0, quantity: 1, subtotal: 0 },
        ],
        ...
        paymentInstructions:
          invoice?.paymentInstructions ?? s.paymentInstructions,
      ...
      const pageContent = (
        <>
          ...
          <div className="flex items-center justify-between mb-5">
            ...
            {formData.visibleFields.companyInfo && (
              <div className="text-right text-sm leading-6">
                ...
                {formData.paymentMethod.address && (
          ...
          <div className="flex justify-between gap-8 mb-6">
            ...
            <div className="text-sm shrink-0">
              <table>
                <tbody>
                  <tr>
                    <td className="font-bold pr-8 pb-2">Invoice:</td>
                    <td className="text-right pb-2">{formData.invoiceNo}</td>
                  </tr>
                  {formData.visibleFields.issueDate && (
                    <tr>
                      <td className="font-bold pr-8 pb-2">Invoice Date:</td>
                  ...
                  {formData.visibleFields.dueDate && formData.dueDate && (
                    <tr>
                      <td className="font-bold pr-8 pb-2">Due Date:</td>
                      <td className="text-right pb-2">
                  ...
                  {formData.billTo.contactName && (
                    <tr>
                      <td className="font-bold pr-8 pb-2">Contact:</td>
                      <td className="text-right pb-2">
                        {formData.billTo.contactName}
                      </td>
                    </tr>
                  ...
                  {formData.billTo.contact && (
                    <tr>
                      <td className="font-bold pr-8">Mobile:</td>
                      <td className="text-right">{formData.billTo.contact}</td>
                    </tr>
                  )}
                </tbody>
              </table>
          ...
          <table className="w-full text-sm">
            <thead>
              <tr className="border-t border-b border-gray-300">
                ...
                <th className="text-center font-semibold py-3 w-20">
                  Quantity
                </th>
            ...
            <tbody>
              {formData.lineItems.map((item, _, list) => (
                <tr key={item.id} className="border-b border-gray-200">
                  <td className={`py-${list.length > 2 ? 3 : 3} pr-4`}>
                    {item.description}
                  </td>
                  <td
                    className={`py-${list.length > 2 ? 3 : 3} text-center text-gray-600`}
                  >
                    {item.quantity}
                  </td>
                  <td
                    className={`py-${list.length > 2 ? 3 : 3} text-right text-gray-600`}
                  >
                    {formatCurrency(item.price, formData.currency)}
                  </td>
                  <td className={`py-${list.length > 2 ? 3 : 3} text-right`}>
          ...
          {(() => {
            ...
            const termsBlock = formData.visibleFields.terms &&
              formData.notes && (
                <div className="text-xs pt-2">
                  <p className="font-semibold mb-1">{formData.termsTitle}</p>
                  <p className="text-gray-600 leading-5 whitespace-pre-line">
                    {formData.notes}
                  </p>
                </div>
              );
              ...
              <table className="text-sm w-64 shrink-0">
                <tbody>
                  {formData.visibleFields.discount && formData.discount > 0 && (
                    <tr>
                      <td
                        className={`py-${formData.lineItems.length > 2 ? 1.5 : 2} pr-6 text-gray-600`}
                      >
                        Discount {formData.discount}%
                      </td>
                      <td
                        className={`py-${formData.lineItems.length > 2 ? 1.5 : 2} text-right`}
                      >
                        -{formatCurrency(discountAmt, formData.currency)}
                      </td>
                    </tr>
                  )}
                  <tr>
                    ...
                    <td
                      className={`py-${formData.lineItems.length > 2 ? 1.5 : 2} text-right`}
                    >
                      {formatCurrency(afterDiscount, formData.currency)}
                    </td>
                  </tr>
                  {formData.visibleFields.sgst && formData.sgst > 0 && (
                    <tr>
                      ...
                      <td
                        className={`py-${formData.lineItems.length > 2 ? 1.5 : 2} text-right`}
                      >
                        {formatCurrency(sgstAmt, formData.currency)}
                      </td>
                    </tr>
                  )}
                  {formData.visibleFields.cgst && formData.cgst > 0 && (
                    <tr>
                      ...
                      <td className={`py-1 text-right`}>
                        {formatCurrency(cgstAmt, formData.currency)}
                      </td>
                    </tr>
                  )}
                  <tr className="border-t border-b border-gray-300">
                    <td
                      className={`py-2 pr-6 font-bold ${formData.paidAmount <= 0 ? "text-base" : ""}`}
                    >
                      Total
                    </td>
                    <td
                      className={`py-2 text-right font-bold ${formData.paidAmount <= 0 ? "text-base" : ""}`}
                    >
              ...
              <div className="flex items-center justify-between gap-6">
                <div className="text-sm text-gray-600 leading-6 mt-2">
                  {formData.paymentInstructions && (
                    <p className="mb-2">{formData.paymentInstructions}</p>
                  )}
                  {formData.visibleFields.bankDetails &&
                        ...
                        <tbody>
                          {formData.paymentMethod.bankDetails
                            ...
                            .map((line, idx) => {
                              ...
                              return (
                                <tr key={idx}>
                                  <td className="font-semibold text-gray-700 pr-6 py-0.5 whitespace-nowrap">
                                    {labelText}
                                  </td>
                ...
                {qrCode && (
                  <div className="shrink-0 flex flex-col items-center gap-2 -mt-4">
                    <img
                      src={qrCode}
                      alt="Payment QR"
                      className="w-28 h-28 object-contain border border-gray-200 rounded p-1"
                    />
                    <p className="text-[10px] text-gray-500 text-center leading-tight">
                      Pay via GPay, PhonePe &amp; more
                    </p>
                  </div>
                )}
              </div>
              ...
              return (
                ...
                >
                  ...
                  >
                    ...
                    {formData.visibleFields.authorizedSignatory &&
                      i === totalPages - 1 && (
                    ...
                    {website &&
                      formData.visibleFields.website &&
                      i === totalPages - 1 && (
            ...
            <Card>
              ...
              <CardContent className="space-y-4">
                ...
                <div className="text-sm text-gray-500">
                  Selected currency will be applied to all amounts in the
                  invoice.
                </div>
              </CardContent>
            </Card>
            ...
            <Card>
              ...
              <CardContent className="space-y-3">
                <div className="flex items-center justify-between">
                  <Label htmlFor="companyInfo" className="cursor-pointer">
                    Company Info
                  </Label>
                  <Switch
                    id="companyInfo"
                ...
                <div className="flex items-center justify-between">
                  <Label htmlFor="clientInfo" className="cursor-pointer">
                    Client Info
                  </Label>
                  <Switch
                    id="clientInfo"
                ...
                <div className="flex items-center justify-between">
                  <Label htmlFor="issueDate" className="cursor-pointer">
                    Issue Date
                  </Label>
                  <Switch
                    id="issueDate"
            ...
            <CardContent className="space-y-4">
              ...
              <div className="p-4 bg-gray-50 rounded-lg space-y-2">
                ...
                <div className="flex justify-between font-bold text-red-600 border-t pt-2">
                  ...
                  <span>
                    {formatCurrency(formData.dueAmount, formData.currency)}
                  </span>
                </div>
                <div className="mt-2">
                  ...
                  <div className="w-full bg-gray-200 rounded-full h-2">
                    <div
                      ...
                      style={{
                        width: `${formData.total > 0 ? Math.min((formData.paidAmount / formData.total) * 100, 100) : 0}%`,
                      }}
                    ></div>
                  </div>
                  <div className="text-xs text-gray-500 mt-1">
                    {formData.total > 0
                      ? ((formData.paidAmount / formData.total) * 100).toFixed(
                          1,
                        )
                      : "0.0"}
                    % paid
