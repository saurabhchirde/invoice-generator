...
export function SettingsPage({
  ...
  const handleLogoUpload = async (e: React.ChangeEvent<HTMLInputElement>) => {
    ...
    try {
      if (isFirebaseEnabled && user) {
        const url = await uploadImage(user.uid, "logo", file);
        update({ logo: url });
      } else {
  ...
  const handleQrUpload = async (e: React.ChangeEvent<HTMLInputElement>) => {
    ...
    try {
      if (isFirebaseEnabled && user) {
        const url = await uploadImage(user.uid, "qrCode", file);
        update({ qrCode: url });
      } else {
        const reader = new FileReader();
        reader.onload = () => {
      ...
      <div className="grid grid-cols-1 lg:grid-cols-2 gap-5">
        ...
        <Card className="flex flex-col gap-0">
          ...
          <CardContent className="space-y-5 pt-4 flex-1">
            <Field
              label="Company Logo"
              hint="Keep under 500 KB for best performance."
            >
              {logoLoading ? (
            ...
            <div className="grid grid-cols-1 sm:grid-cols-2 gap-3">
              <Field
                ...
                hint="Use {YYYY} for year. e.g. #, INV-, INV-{YYYY}-"
              >
                <Input
              ...
              <Field
                ...
                hint="Next invoice will auto-increment from here."
              >
                <Input
                  type="number"
                  min="1"
        ...
        <Card className="flex flex-col gap-0">
          <CardHeader className="pb-2">
            ...
            <p className="text-sm text-gray-500">
              Bank account, instructions and QR shown on invoices.
            </p>
          </CardHeader>
          <CardContent className="space-y-5 pt-4 flex-1">
            ...
            <div className="border-t border-gray-100 pt-5 space-y-5">
              ...
              <Field
                ...
                hint="UPI / GPay / PhonePe QR — shown on every invoice."
              >
                {qrLoading ? (
                  <div className="flex items-center justify-center gap-2 w-full h-24 border-2 border-dashed border-gray-200 rounded-xl text-sm text-gray-400 bg-gray-50">
                    <Loader2 className="w-4 h-4 animate-spin" />
